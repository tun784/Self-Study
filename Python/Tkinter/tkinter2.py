...
def display_customers(customers):
    ...
    tk.Label(customer_frame, text="Thông tin khách hàng", font=('Arial', 12, 'bold')).grid(row=0, column=0, columnspan=2, padx=5, pady=5)
    
    for idx, customer in enumerate(sorted(customers, key=lambda x: x['tong_tien'], reverse=True), start=1):
        ...
        delete_button = tk.Button(customer_frame, text="Xóa", command=lambda idx=idx-1: delete_customer(idx-1))
...
tk.Button(root, text="Thêm khách hàng", command=add_customer).grid(row=3, column=0, columnspan=2, pady=5)
...
search_frame.grid(row=5, column=0, columnspan=2, padx=5, pady=5, sticky='w')

tk.Label(search_frame, text="Tìm kiếm khách hàng").grid(row=0, column=0, padx=5, pady=5, sticky='w')
search_entry = tk.Entry(search_frame, width=40)
search_entry.grid(row=0, column=1, padx=5, pady=5)
tk.Button(search_frame, text="Tìm kiếm", command=search_customer).grid(row=0, column=2, padx=5, pady=5)

# Listbox hiển thị kết quả tìm kiếm
tk.Label(root, text="Kết quả tìm kiếm:").grid(row=6, column=0, padx=5, pady=5, sticky='w')
